...
export default function EventListModal({
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed inset-4 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 z-50 sm:w-[95%] sm:max-w-[520px] sm:max-h-[85vh] overflow-hidden scale-in flex flex-col">
        <div className="bg-slate-900/95 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden flex-1 flex flex-col max-h-full">
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 overflow-y-auto flex-1">
            {sortedEvents.length === 0 ? (
              <div className="py-8 sm:py-12 text-center">
                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto rounded-xl sm:rounded-2xl bg-gradient-to-br from-cyan-500/20 to-cyan-500/5 flex items-center justify-center mb-3 sm:mb-4">
                  <svg className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
                </div>
                <p className="text-white/40 mb-1 text-sm sm:text-base">No events for this day</p>
                <p className="text-white/20 text-xs sm:text-sm">Click the button below to add one</p>
              </div>
            ) : (
              <div className="space-y-2 sm:space-y-3">
                {sortedEvents.map((event, index) => (
                  <div
                    ...
                    className={`
                      glass-card p-3 sm:p-4 transition-all duration-300 fade-in group
                    ...
                    <div className="flex items-start gap-2 sm:gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-1 sm:mb-1.5">
                          <span className="text-base sm:text-lg">{CATEGORY_CONFIG[event.category].icon}</span>
                          <h3 className={`
                            font-semibold text-sm sm:text-base text-white truncate
                        ...
                        {/* Meta */}
                        <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-white/50 mb-1.5 sm:mb-2 flex-wrap">
                        {event.time && (
                            <span className="font-mono bg-white/5 px-1.5 sm:px-2 py-0.5 rounded">
                    ...
                    {event.description && (
                      ...
                      <button
                        onClick={() => onEditEvent(event)}
                            className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs rounded-lg font-medium bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 transition-all hover:bg-cyan-500/30"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(event.id)}
                            className={`
                              px-2.5 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs rounded-lg font-medium transition-all
                      ...
                      {/* Color indicator */}
                      <div
                        className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 shadow-lg"
                        style={{ 
          ...
          {/* Footer */}
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-white/10 flex gap-2 sm:gap-3 justify-between bg-white/[0.02] flex-shrink-0">
            <div className="text-[10px] sm:text-xs text-white/30 self-center">
            ...
            <div className="flex gap-2 sm:gap-3">
              ...
              <button onClick={onAddEvent} className="btn-primary text-xs sm:text-sm px-3 sm:px-5 py-2 sm:py-2.5">
